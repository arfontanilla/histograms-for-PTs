...
fig.suptitle("Histograms with Correct Smooth Curve Overlay", fontsize=16)
...
# --- Function to plot histogram and curve ---
def plot_hist_and_curve(ax, data, bins, shape_param, scale_param, color, title, xlabel):
    """Plots a histogram and its corresponding scaled gamma distribution curve."""
    ...
    # Calculate the y-values (PDF) for the curve using the gamma distribution
    y_pdf = gamma.pdf(x_curve, a=shape_param, scale=scale_param)

    # Scale the PDF and plot the curve
...
data1 = np.clip(data1, 0, 100) # Clip data to a realistic range (0-100)
plot_hist_and_curve(axes[0, 0], data1, bin_count, subtle_skew_shape, scale1,
                    colors[0], "Student Test Scores", "Score (out of 100)")

# --- Version 2: Daily Coffee Shop Customers ---
scale2 = 8
data2 = np.random.gamma(shape=subtle_skew_shape, scale=scale2, size=num_samples)
plot_hist_and_curve(axes[0, 1], data2, bin_count, subtle_skew_shape, scale2,
                    colors[0], "Daily Coffee Shop Customers", "Number of Customers")

# --- Version 3: Duration of Customer Service Calls ---
scale3 = 0.33
data3 = np.random.gamma(shape=subtle_skew_shape, scale=scale3, size=num_samples)
plot_hist_and_curve(axes[1, 0], data3, bin_count, subtle_skew_shape, scale3,
                    colors[0], "Duration of Customer Service Calls", "Duration (minutes)")

# --- Version 4: Commute Times to Work ---
scale4 = 1.67
data4 = np.random.gamma(shape=subtle_skew_shape, scale=scale4, size=num_samples)
plot_hist_and_curve(axes[1, 1], data4, bin_count, subtle_skew_shape, scale4,
                    colors[0], "Commute Times to Work", "Commute Time (minutes)")
