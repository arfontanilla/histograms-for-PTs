...
# Add flat line
flat_height = np.mean(counts)
axes[0, 0].axhline(y=flat_height, color='black', linestyle='-', linewidth=3)
...
# Add flat line
flat_height = np.mean(counts)
axes[0, 1].axhline(y=flat_height, color='black', linestyle='-', linewidth=3)
...
# Add flat line
flat_height = np.mean(counts)
axes[1, 0].axhline(y=flat_height, color='black', linestyle='-', linewidth=3)
...
# Add flat line
flat_height = np.mean(counts)
axes[1, 1].axhline(y=flat_height, color='black', linestyle='-', linewidth=3)

# Adjust layout and save the figure
plt.tight_layout()
plt.savefig("uniform_histograms_with_curves.png", dpi=300, bbox_inches='tight')
plt.show()