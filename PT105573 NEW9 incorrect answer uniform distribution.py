...
mean, std = 3.5, 1.5  # Centered in middle with wide spread
y = norm.pdf(x, mean, std) * num_samples * (bins[1]-bins[0])
axes[0, 0].plot(x, y, color='black', linewidth=2)
...
mean, std = 2, 1.2  # Centered in middle with wide spread
y = norm.pdf(x, mean, std) * num_samples * (bins[1]-bins[0])
axes[0, 1].plot(x, y, color='black', linewidth=2)
...
# Add bell curve
x = np.linspace(9.5, 17.5, 100)
mean, std = 13.5, 2  # Centered in middle with wide spread
y = norm.pdf(x, mean, std) * num_samples * (bins[1]-bins[0])
axes[1, 0].plot(x, y, color='black', linewidth=2)
...
mean, std = 4.5, 2.5  # Centered in middle with wide spread
y = norm.pdf(x, mean, std) * num_samples * (bins[1]-bins[0])
axes[1, 1].plot(x, y, color='black', linewidth=2)

# Adjust layout and save the figure
plt.tight_layout()
plt.savefig("uniform_histograms_with_bell_curves.png", dpi=300, bbox_inches='tight')
plt.show()