...
neg_skewness = -5  # Negative skewness for incorrect curves

# Custom colors from hex codes
colors = ['#838EF0', '#60B2DE', '#19C9D6', '#03A28D']

# Function to add INCORRECT negative skew curve to a histogram
def add_incorrect_curve(ax, data, color):
    # Create a negatively skewed distribution (incorrect for this data)
    x = np.linspace(min(data), max(data), 1000)
    
    # Scale parameters to roughly match the data range
    loc = np.mean(data) + np.std(data)  # Shift right to make it clearly wrong
    scale = np.std(data)/2
    
    # Generate negatively skewed PDF
    y = skewnorm.pdf(x, neg_skewness, loc, scale)
...
add_incorrect_curve(axes[0, 0], data1, colors[0])
...
add_incorrect_curve(axes[0, 1], data2, colors[1])
...
add_incorrect_curve(axes[1, 0], data3, colors[2])
...
add_incorrect_curve(axes[1, 1], data4, colors[3])
...
plt.tight_layout()
plt.savefig("incorrect_negatively_skewed_curves.png", dpi=300, bbox_inches='tight')
plt.show()